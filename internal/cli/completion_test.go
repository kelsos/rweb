...
func TestConfigHome(t *testing.T) {
	...
	if got := configHome(); got != "/custom/cfg" {
		t.Errorf("configHome with XDG set = %q", got)
	}
	t.Setenv("XDG_CONFIG_HOME", "")
	home := t.TempDir()
	t.Setenv("HOME", home)
	if got := configHome(); got != filepath.Join(home, ".config") {
		t.Errorf("configHome default = %q", got)
	}
}

// fakeRoot is a minimal command that can generate completion scripts.
func fakeRoot() *cobra.Command {
	c := &cobra.Command{Use: "rweb"}
	c.AddCommand(&cobra.Command{Use: "up", Run: func(*cobra.Command, []string) {}})
	return c
}
...
func TestInstallFishCompletion(t *testing.T) {
	...
	path := filepath.Join(home, ".config", "fish", "completions", "rweb.fish")
...
func TestInstallBashCompletion(t *testing.T) {
	...
	path := filepath.Join(home, ".local", "share", "bash-completion", "completions", "rweb")
	assertNonEmptyFile(t, path)
}
...
func TestInstallZshCompletionFallback(t *testing.T) {
	...
	assertNonEmptyFile(t, filepath.Join(home, ".zsh", "completions", "_rweb"))
}
...
func TestInstallIsIdempotentUpdate(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	path := filepath.Join(home, ".zsh", "completions", "_rweb")
	if err := installZshCompletion(fakeRoot()); err != nil {
		t.Fatal(err)
	}
