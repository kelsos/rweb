...
// completions after upgrading rweb.
func completionInstallCmd(root *cobra.Command) *cobra.Command {
	return &cobra.Command{
		...
		Long: "Install the shell completion script into a per-user directory (no root).\n" +
			...
			"rweb to refresh the completions.",
...
func installZshCompletion(root *cobra.Command) error {
	...
	path, err := writeCompletion(dir, "_rweb", root.GenZshCompletion)
	if err != nil {
		return err
	}
...
func installBashCompletion(root *cobra.Command) error {
	...
	path, err := writeCompletion(dir, "rweb", func(w io.Writer) error {
...
func installFishCompletion(root *cobra.Command) error {
	...
	path, err := writeCompletion(dir, "rweb.fish", func(w io.Writer) error {
...
// zshCompletionDir picks a per-user directory for the _rweb completion. It
...
func isWritableDir(dir string) bool {
	...
	f, err := os.CreateTemp(dir, ".rweb-wtest-*")
	if err != nil {
		return false
	}
	name := f.Name()
